...
class _EditCatScreenState extends State<EditCatScreen> {
  ...
  Future<void> _pickImage() async {
    ...
    try {
      ...
      final photoUrl = await storageService.uploadFile(
        File(image.path),
        storageService.getStoragePath(currentUser.uid, 'cat'),
      );
  ...
  Future<void> _saveCat() async {
    ...
    try {
      ...
      final cat = CatModel(
        id: widget.cat?.id ?? const Uuid().v4(),
      ...
      if (widget.cat == null) {
        await firestoreService.createCat(cat);
      } else {
        await firestoreService.updateCat(cat);
      }
